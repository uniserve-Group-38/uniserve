...
export function ServiceCard({
  ...
  const displayImage =
    imageUrl ||
    DEFAULT_CATEGORY_IMAGES[category as ServiceCategory] ||
  ...
  const rotationClass =
    index !== undefined && index % 2 === 0 ? "rotate-[1deg]" : "rotate-[-1deg]";

  return (
    <Link
      href={`/services/${id}`}
      className="block h-full cursor-pointer focus:outline-none rounded-none"
    >
      <article
        className={`group relative border-4 border-black bg-white shadow-[8px_8px_0_0_#000] hover:translate-x-[-4px] hover:translate-y-[-4px]
      ...
      >
        ...
        <div className="absolute inset-0 bg-black/95 flex flex-col justify-between text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none group-hover:pointer-events-auto z-20">
          <div className="p-6 flex flex-col grow">
            <div className="text-[10px] font-black tracking-widest mb-2 text-cyan-300">
              {category.toUpperCase()}
            </div>
            <h3 className="text-2xl font-black mb-4 line-clamp-2 leading-tight">
              {title}
            </h3>
            <p className="font-bold mb-4 leading-relaxed text-sm line-clamp-4 text-white/90">
              {description}
            </p>
            ...
            <div className="flex items-end justify-between mt-auto mb-4 gap-2">
              ...
              {/* Price */}
              <div className="font-black text-xl text-cyan-300 shrink-0">
                {price || "FREE"}
              </div>
            </div>
          </div>
